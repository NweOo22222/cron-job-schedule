function __fixedManually() {
  ...
  if (
    code.includes(fixes[0]) &&
    code.includes(fixes[1]) &&
    code.includes(fixes[2])
  ) {
...
if (vid.length === 11 || vid.includes("youtu")) {
  ...
  require("./src/_helpers")
    ...
    .then((data) => {
      console.log(
        '[ OK ] ytdl-core:getVideoID - title="%s" formats.length=%d',
        data.title,
        data.formats.length
      );
      if (!data.formats.length) {
        return getDashUrl(vid).then((responseData) =>
...
function getDashUrl(id) {
  ...
  return axios
    .get(
      `https://www.youtube.com/get_video_info?video_id=${id}&eurl=https%3A%2F%2Fyoutube.googleapis.com%2Fv%2Fonz2k4zoLjQ&html5=1&c=TVHTML5&cver=6.20180913`
    )
    .then(({ data }) => {
